...
const Page = ({ params }) => {
  ...
  const handleApply = () => {
    ...
    if (session === null || session === undefined) {
      ...
      toast({
        variant: "destructive",
        description: "You are Not allowed to Apply . You are a Employer",
      });
  ...
  return company && job ? (
    <div className="lg:mx-32">
      <Card className="w-full">
        <CardHeader className="flex flex-col max-sm:gap-2 gap-4 bg-primary border text-white rounded-md">
          <div className="flex justify-between">
            <div className="flex flex-col gap-2">
              <h1 className="font-bold text-3xl max-sm:text-xl max-md:text-2xl">
                {" "}
                {job.title}
              </h1>
              <p className="font-semibold max-sm:text-sm max-md:text-lg text-xl">
                {company.companyName}
              </p>
            </div>
            <div>
              <a href="#apply" onClick={handleApply}>
                <Button className="text-primary max-sm:text-sm hover:bg-gray-300 text-xl bg-white">
          ...
          {job.location === "Relocate" ? (
            <div className="flex flex-row items-center gap-2">
              ...
              <h1 className="text-lg font-semibold max-md:text-md text-white">
                {job.location}
              </h1>
            </div>
          ) : (
            <div className="flex flex-row items-center gap-2">
              ...
              <h1 className="text-lg  max-md:text-md font-semibold text-white">
                {job.location}
              </h1>
            </div>
          )}
          ...
          <div className="flex gap-20  max-md:text-md max-sm:text-sm max-md:gap-4 border-2 p-2 px-4 w-fit max-sm:flex-wrap border-white rounded-md">
            <div className="flex gap-2 items-center">
              ...
              <h1 className="text-lg font-semibold max-md:text-md max-sm:text-sm text-white">
                {job.duration}
              </h1>
            </div>
            <div className="flex gap-2 items-center">
      ...
      <Card className="mt-2 w-full border-2 mb-4 border-gray-200">
        ...
        <CardContent className="flex flex-col gap-2 mt-2">
          ...
          {company.about && (
            <div className="flex flex-col md:ml-2 mt-8 gap-2">
          ...
          <ul>
            ...
            {job.skills.map((skill, index) => (
              <li
                className="md:ml-4  text-blue-950 disc-none text-md "
                key={index}
              >
